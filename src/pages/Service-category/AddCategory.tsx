...
const AddCategory: React.FC<AddCategoryProps> = ({ isOpen, onClose, onSubmit }) => {
      ...
      try {
        ...
        do {
          const response = await axiosInstance.get(
            `/service-categories?page=${currentPage}`,
            config
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.post("/service-categories", newCategory, config);
      ...
      <div className="bg-white dark:bg-white text-black dark:text-black w-96 p-6 rounded-md shadow-lg">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <p className="text-xs text-green-700 mb-4">
            If a parent name is not selected, the category name will be used as the parent name.
          </p>
          <div className="mb-4">
