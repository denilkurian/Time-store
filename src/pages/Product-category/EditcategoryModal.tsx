...
const EditCategoryModal: React.FC<EditCategoryModalProps> = ({ isOpen, onClose, onSubmit, category, successMessage, error }) => {
  ...
  const fetchAllParentCategories = async () => {
    try {
      ...
      do {
        const response = await axiosInstance.get(`/product-categories?page=${currentPage}`);
