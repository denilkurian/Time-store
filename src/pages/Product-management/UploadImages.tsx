...
import { setFiles, setLoading, setError,removeFile, setDisplayPicture } from "../../redux/features/ProductImage/imageSlice";
...
const UploadImages: React.FC<UploadImagesProps> = ({  }) => {
  ...
  const [dpLoading] = useState<string | null>(null);
  ...
  const loadInitialImages = async () => {
    try {
      ...
      const dpResponse = await axiosInstance.get(`/products/${productId}/dp`);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const uploadPromises = allowedFiles.map(async (file) => {
      ...
      formData.append("data[attributes][product_image]", file);
    ...
    try {
      ...
      if (successCount > 0 ) {
        const message = `${successCount} images uploaded Successfully`;
        showToast({
          message: message,
        ...
        const message = `${successCount} files uploaded. ${failureCount} failed.`;
        showToast({
          message: message,
  ...
  const handleSetDisplayPicture = async (id: string) => {
    try {
      setInitialLoader({ ...initialLoader, type: 'set', id: id })
      const response = await axiosInstance.post(`/products/${productId}/dp/${id}`);
      ...
      setInitialLoader({ id: null, type: null });
    }
  };

  const handleDeleteDisplayPicture = async (fileId: string) => {
    try {
      setInitialLoader({ ...initialLoader, type: 'remove', id: fileId });
      const response = await axiosInstance.delete(`/products/${productId}/dp`);
  ...
  const handleDeleteImage = async (id: string) => {
    try {
      // dispatch(setLoading(true));
      // setDeleteLoading(id);
      setInitialLoader({ ...initialLoader, type: 'delete', id: id })
      const response = await axiosInstance.delete(`/product_images/${id}`);
      ...
      setInitialLoader({ type: null, id: null });
    }
  };
  ...
  return (
    <div className="container mx-auto p-6 min-h-[250px]">
              ...
              files.map((file) => (
                ...
                >
                  ...
                  <div className="relative w-full h-40">
                    ...
                    {/* "Setting..." Overlay */}
                    {initialLoader.id === file.id && initialLoader.type === 'set' && (
                      <div className="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
                        <p className="text-white text-sm font-semibold animate-pulse">Setting...</p>
                      </div>
                    )}
                    {initialLoader.id === file.id && initialLoader.type === 'delete' && (
                      <div className="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
                        <p className="text-red-500 text-sm font-semibold animate-pulse">Deleting...</p>
                      </div>
                    )}
                    {initialLoader.id === file.id && initialLoader.type === 'remove' && (
                      <div className="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
                        <p className="text-red-500 text-sm font-semibold animate-pulse">Removing...</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-3 flex flex-col gap-2">
                    ...
                    {displayPictureId == file.id && (
                      <button
                        onClick={() => handleDeleteDisplayPicture(file.id)}
